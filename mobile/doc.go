...
// Package geth contains the simplified mobile APIs to mesh-chain.
//
// The scope of this package is *not* to allow writing a custom Ethereum client
// with pieces plucked from mesh-chain, rather to allow writing native dapps on
...
// from the mesh-chain repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original mesh-chain version.
